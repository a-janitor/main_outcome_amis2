...
d <- d %>%
  mutate(
    ...
    mt_info_t1t2 = as.integer(
      !is.na(mt_mal_status_t1t2)
    )
  )
...
for (v in mal_base) {
  ...
  d[[harm_var]] <- case_when(
    d[[t1_var]] == 1 |
      d[[t1t2_var]] == 1 ~ 1L,
    
    is.na(d[[t1_var]]) &
      is.na(d[[t1t2_var]]) ~ NA_integer_,
  ...
  fpar(
    ...
    ftext(
      paste0(
        ...
        "subscales. Harmonized maltreatment indicators were coded positive ",
        "when the corresponding indicator was positive at T1 or in the ",
        "cumulative T1–T2 variable, negative when information was available ",
        "but neither indicator was positive, and missing when both indicators ",
        "were missing. Inferential tests are reported for demographic ",
        "characteristics, psychopathology, and assessment coverage, but not ",
        "for maltreatment characteristics used to define the trajectory classes. ",
